...
**Check**: Ensure the policy engine has the right rules::
    ...
    curl -X GET localhost:1789/policies/classification/rules

For example::

  nicira@Ubuntu1204Server:/opt/stack/congress$ curl -X GET localhost:1789/policies/classification/rules
  {
    "results": [
      {
        "comment": "None",
...
for the list of tables occurring in the rules::
    ...
    curl -X GET localhost:1789/policies/classification/tables

For example::

  nicira@Ubuntu1204Server:/opt/stack/congress$ curl -X GET localhost:1789/policies/classification/tables
  {
    "results": [
      {
        "id": "nova:servers"
...
If they are not empty, it points to a problem with the rules::
    ...
    curl -X GET localhost:1789/policies/classification/tables/<table-id>/rows
...
rows (each of which represents a port), and each row has 16 columns::
  ...
  nicira@Ubuntu1204Server:/opt/stack/congress$ curl -X GET localhost:1789/policies/classification/tables/neutron:ports/rows
  {
    "results": [
      {
        "data": [
...
a different group::
    ...
    nicira@Ubuntu1204Server:/opt/stack/congress$ curl -X GET localhost:1789/policies/classification/tables/group/rows
    {
      "results": [
        {
          "data": [
...
describes how the rows of that table were computed::
    ...
    curl -X GET localhost:1789/policies/classification/tables/<table-id>/rows?trace=true
...
as a string and be quite large.::
  ...
  nicira@Ubuntu1204Server:/opt/stack/congress$ curl -X GET localhost:1789/policies/classification/tables/error/rows?trace=true
  {
    "results": [],
...
the documentation says should be exported::
    ...
    curl -X GET localhost:1789/data-sources/<ds-name>/tables
...
the right tables::
    ...
    nicira@Ubuntu1204Server:~$ curl -X GET localhost:1789/data-sources/neutron/tables
    {
      "results": [
        {
...
datasource are correct::
    ...
    curl -X GET localhost:1789/data-sources/<ds-name>/tables/<table-name>/rows
...
the *data* key::
  ...
  nicira@Ubuntu1204Server:/opt/stack/congress$ curl -X GET localhost:1789/data-sources/neutron/tables/networks/rows
  {
    "results": [
      {
        "data": [
...
D is D:T, e.g. the *networks* table for service *neutron* is named *neutron:networks*::
  ...
  curl -X GET localhost:1789/policies/classification/tables/<ds-name>:<table-name>/rows
  curl -X GET localhost:1789/data-sources/<ds-name>/tables/<table-name>/rows
